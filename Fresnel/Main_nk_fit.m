...
% Exciton removed. Kept E0 anchored at 6.05 and eps_inf at 4.0.
x_guess = [6.05, 0.05, 30.0, 8.70, 0.60, 2.0, 4.0];

options = optimset('Display', 'iter', 'MaxIter', 3000, 'MaxFunEvals', 5000, 'TolX', 1e-4);

disp('Starting the Adachi Pre-Fit Optimization (No Exciton)...');
...
% Unpack the 7 winning parameters
...
% ========================================================
[n_sim, k_sim] = adachi_ext(Energy_eV, best_E0, best_Gamma0, best_A0, best_E1, best_Gamma1, best_B1, best_eps_inf);
...
ylabel('Refractive Index (n)'); title('Adachi Pre-Fit Match (No Exciton)'); legend('Location', 'northeast'); grid on; xlim([187, 400]);
...
xlabel('Wavelength (nm)'); ylabel('Extinction Coeff (k)'); legend('Location', 'northeast'); grid on; xlim([187, 400]);
